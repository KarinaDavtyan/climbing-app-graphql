...
const createSportPost = async ({ data }) => {
  ...
  try {
    ...
    if (sport_post.insertedId) {
      ...
      const targetUser = await users.updateOne({ _id: new ObjectId(user_id) }, { $push: { sport_posts: sport_post.insertedId } });
      ...
      if (route.insertedId) {
        ...
        const targetClimbingArea = await climbing_areas.updateOne({ name: climbing_area_name }, { $push: { routes: route.insertedId, sport_posts: sport_post.insertedId } });
      }
